...
psi_HPT_grid = zeros(size(r_HPTm_grid));
r_IPTm = 0.4;
...
figure;
contourf(r_HPTm_grid, psi_HPT_grid, N_HP_grid, 20, 'LineColor', 'none');
shading interp;
h = colorbar;
...
saveas(gcf, 'HPMeanRadius_v_Loading_for_Speed.eps', 'epsc');

r_HPTm = 0.4;
N_IP = 158.3;
[r_IPTm_grid, N_IP_grid] = meshgrid(r_IPTm_range, N_IP_range);
psi_IPT_grid = zeros(size(r_IPTm_grid));
...
figure;
contourf(r_IPTm_grid, psi_IPT_grid, N_IP_grid, 20, 'LineColor', 'none');
shading interp;
h = colorbar;
...
set(gca, "TickLabelInterpreter", 'latex');
saveas(gcf, 'IPMeanRadius_v_Loading_for_Speed.eps', 'epsc');
